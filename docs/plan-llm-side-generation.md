# Development Plan — LLM Side Generation (remote API key, Claude Code/Codex CLI bridge, local Ollama)
...
Today the only way to get a side into SideQuest is the bundled ones or a hand-written/hand-exported JSON file via `dashboard:import-side`. This plan adds a **"Generate" button** next to **"Import"** in the Sides gallery: the user describes what they want ("a 5-minute posture side for people who sit all day"), an LLM produces a side, validated through the exact same sanitizer as manual import, and it lands in the gallery like any other side.
...
- **Text-only generation.** No mascot *image* generation — a generated side has no image-gen mascot; it falls back to the existing hash-based accent color (`plan-marketplace-sides.md` § 3.5) or the user assigns one of the bundled mascots afterward. Explicitly deferred, not forgotten: no provider here can produce an image at all (Anthropic's Messages API and the OpenAI Chat Completions endpoint used by `openai-api.ts` are both text-only; the CLI bridges and Ollama's `/api/generate` are text-only too — real image generation would need a dedicated endpoint like OpenAI's `images/generations`, a separate integration not built).
  - **2026-08-27, lightweight follow-up**: an optional "describe your mascot" textarea was added to the Generate modal. It doesn't generate an image — the description is folded into the same text prompt, and the model may return an extra `mascotIdea` string (`parseSideJson`, one sentence, capped at 300 chars) describing a mascot *concept* in words. Shown once in the side editor next to the mascot picker (`sideEditor.aiMascotIdea`, magenta "AI" styling to match the `generated` side badge) so the user can pick a matching mascot by hand. Never persisted to the side/DB — it's a one-shot UI hint, not a `SideMascot`.
- **Generated output must conform to the same schema as manual import** (`{ name, exercises: [{ id, label, durationSec, category }] }`) — no new side shape, no LLM-authored code ever executed (sides stay pure declarative JSON, same rationale already documented in `sides.ts` and in `plan-marketplace-sides.md` § 3.6).

## 3. Technical architecture

### 3.1 Shared side validator (prerequisite refactor)

`dashboard:import-side` in `packages/app/src/main/index.js` currently inlines the JSON shape validation/sanitization (name/exercises coercion, id fallback, duration clamping). Extract it to `packages/core/src/sides.ts` as `parseSideJson(data: unknown): { name: string; exercises: Exercise[] } | { error: string }`, so both manual import and LLM generation run through **one** sanitizer — untrusted input either way, same rules (duration bounds, id fallback via `randomUUID()`, no unexpected keys carried through, exercise count capped — e.g. 12 max, new documented constant alongside `MAX_MASCOT_IMAGE_BYTES`'s style). `dashboard:import-side` is refactored to call it; behavior unchanged.

`SideSource` (`packages/core/src/sides.ts`) gains a fourth value: `"generated"` — distinct provenance badge in the gallery ("Généré par IA" / "AI-generated"), reusing the badge mechanism already built for `bundled`/`imported`/`custom` (`plan-marketplace-sides.md` § 3.3).

### 3.2 Provider abstraction (`packages/core`)

New `packages/core/src/llm/` :

- `types.ts` — `interface LlmProvider { generate(prompt: string, opts: LlmGenerateOptions): Promise<string> }`, returns raw text (the provider's best attempt at JSON), never a parsed `Side` — parsing/sanitizing is `parseSideJson`'s job (3.1), kept separate so every provider is exercised by the same validation path in tests.
...
- `anthropic-api.ts` / `openai-api.ts` — direct `fetch()` to the Messages / Chat Completions endpoint with the user's API key in the request header. No new SDK dependency, consistent with the project's existing "no new dependency where a direct call suffices" pattern (`plan-marketplace-sides.md` § 3.1 on skipping zod).
...
API keys **never** go through `packages/core/storage.ts` / SQLite — same principle already applied to mascot base64 ("never stored as-is", `plan-marketplace-sides.md` § 3.1). Instead:
...
- `dashboard:generate-side` (prompt: string) → resolves the configured provider, decrypts the key if needed, calls `generate()` with a timeout, runs the result through `parseSideJson` (3.1), on success `storage.createSide(name, exercises, { source: "generated" })` — mirrors `dashboard:import-side`'s `{ imported, error, plan }` shape as `{ generated, error, plan }`.

### 3.5 UI

- **Sides gallery** (`dashboard/index.html` `#panel-sides`): new `#generate-side-btn` next to `#import-side-btn`. Click opens a small modal (textarea prompt + optional exercise-count hint), disables the button and shows a spinner while in flight, calls `dashboardAPI.generateSide(prompt)`, then reuses `importSide()`'s existing success path (toast + grid refresh) on success. If `llmProvider === "none"`, the button stays visible (progressive discovery, consistent with how the Claude Code hook install button is always shown with actionable state rather than hidden) but routes straight to the Settings panel with a hint instead of opening the prompt modal.
...
**Included**
- Shared side-JSON validator reused by both import and generation (3.1)
...
- "Generate" button in the Sides gallery + AI provider section in Settings (3.5)
...
1. **Sprint 1 — done, 2026-08-27** — `parseSideJson(data)` extracted into `packages/core/src/sides.ts` (`{name, exercises} | {error: ParseSideJsonError}`, new `MAX_SIDE_EXERCISES = 12` cap), `dashboard:import-side` refactored to call it (tightens behavior slightly: an empty-exercises or >12-exercises file is now rejected, wasn't before). `SideSource` gains `"generated"`. `Settings` gains `llmProvider` (renamed `LlmProviderId` in core to avoid a name collision with the `LlmProvider` *interface* added in Sprint 2 — same word, two different things: one is a settings string union, the other is the provider object shape), `anthropicModel`, `openaiModel`, `ollamaBaseUrl`, `ollamaModel` — additive to the existing key/value `settings` table, no migration needed (unlike `sides`, `Settings` was never column-based). 13 new tests in `sides.test.ts` (valid/invalid shapes, id/duration fallback, empty, too-many, non-object input).
2. **Sprint 2 — done, 2026-08-27** — New `packages/core/src/llm/`: `types.ts` (`LlmProvider`/`LlmGenerateOptions`), `prompt.ts` (`buildSidePrompt`, shared template, 4–12 exercises / 10–120s bounds baked in), `generate-side.ts` (`generateSide()` — one retry on invalid JSON, no retry on a provider-level error since retrying the same broken key/connection is pointless), `anthropic-api.ts`/`openai-api.ts` (direct `fetch()`, no SDK — default model corrected mid-build from a guessed `claude-sonnet-4-5` to the actual current `claude-sonnet-5`), `ollama.ts` (+ `listOllamaModels`). No new dependency — native `fetch`/`AbortSignal.timeout` (Node ≥20, matches `engines`). 21 new tests (`llm-generate-side.test.ts`, `llm-providers.test.ts` with `fetch` mocked via `vi.stubGlobal`).
...
6. **Sprint 6 — done, 2026-08-27** — Error i18n strings (FR/EN) for empty/too-many-exercises/invalid-LLM-JSON/provider-error/not-configured, all routed through one `sideParseErrorKey()` mapper shared by import and generation. `"Généré par IA"` gallery badge (magenta, alongside the existing Officiel/blue and Importé/gold). Full end-to-end live verification: real `claude-cli` generation from the actual running app (not a mock) produced a 6-exercise French side ("Respiration & Épaules Détente") that landed in the gallery with the correct badge — see the commit for the exact flow tested.
...
- **Verify the packaged-build asset path** for `llm-credentials.js` and `packages/core/dist/llm/` the same way `plan-marketplace-sides.md` flagged for `listBundledSides()` — only exercised in dev mode so far.
