...
- **UI** (`dashboard/renderer.js` + `index.html`): the picker shows **all** bundled mascots, not theme-filtered like the Settings page's picker — a pack's identity shouldn't depend on which dashboard skin happens to be active. Clicking the already-active tile clears it (toggle, same convention as the gallery's Activate/Deactivate button) rather than needing a separate "clear" affordance. A pack's existing custom mascot (from import or from this picker) shows as its own active tile, also clickable to clear.
...
Verified live: a new pack's editor renders all 8 tiles (7 bundled + "+"), picking one shows exactly one active tile and updates the gallery thumbnail, re-clicking it clears back to zero active tiles. The file-copy path (`storeMascotFile`) was verified against a real ~1.1 MB PNG outside Electron (byte-identical copy, correct extension/UUID naming) since a native OS file-picker dialog can't be scripted the same way as DOM clicks — the dialog invocation itself reuses the exact `dialog.showOpenDialog` pattern already proven by 3.2's import flow, so risk there is low but this specific combination wasn't clicked through by a human.
