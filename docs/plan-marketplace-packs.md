...
### 3.8 Pack-level mascot picker — done, 2026-08-27

Before this, the only way to give a pack its own mascot was 3.2's import flow — hand-craft a JSON file with a base64-embedded image. Now the pack editor (custom/imported/generated packs — bundled packs stay read-only, same treatment as their name/exercise fields) has a "Mascotte" card: every bundled mascot as a selectable tile, plus a **"+ Add"** tile to upload your own image file directly, no JSON required.

- **`packages/core/src/storage.ts`**: `updatePlan()` gains a third optional field, `mascot?: PackMascot | null` — `null` explicitly clears it (falls back to the global mascot), an absent key leaves it untouched, a `PackMascot` replaces it. Needed a `"mascot" in partial"` check rather than relying on the existing spread-based merge, since `undefined` (not provided) and `null` (clear it) have to mean different things here.
- **`packages/app/src/main/index.js`**: three new IPC handlers. `dashboard:set-plan-mascot-bundled` resolves one of the app's own bundled mascot PNGs to an absolute path (`ASSETS_DIR/mascots/<id>.png`) and assigns it directly — no file copy needed, it's already shipped with the app. `dashboard:set-plan-mascot-custom` opens a native file dialog (png/jpg/webp, same 3 MB cap as import) and copies the chosen file into `userData/custom-mascots/` via a new `storeMascotFile()` — structurally the same as 3.2's `decodeMascotImage()`, just reading a real file instead of decoding base64 (no base64 round-trip needed when a native file path is already in hand). `dashboard:clear-plan-mascot` resets to `null`. All three best-effort clean up the *previous* custom mascot file via a new shared `cleanupCustomMascotFile()` helper (`dashboard:delete-plan`'s inline cleanup was refactored to reuse it too).
- **UI** (`dashboard/renderer.js` + `index.html`): the picker shows **all** bundled mascots, not theme-filtered (a pack's identity shouldn't depend on which dashboard skin happens to be active). Clicking the already-active tile clears it (toggle, same convention as the gallery's Activate/Deactivate button) rather than needing a separate "clear" affordance. A pack's existing custom mascot (from import or from this picker) shows as its own active tile, also clickable to clear.
- **The "+" tile is deliberately named generically** ("+ Add", not "+ Upload"/"+ Import") to leave room for a second option later — generating a mascot image via AI, alongside the already-shipped [plan-llm-pack-generation.md](plan-llm-pack-generation.md) pack-*content* generation — without a rename. Not built yet; today "+" only opens the file dialog.
- **The Settings page's own global mascot picker was removed** (`#mascot-options`, `renderMascotOptions()`) — mascot choice is now a *pack* property, not a separate global preference, so keeping both would have been redundant and confusing about which one actually wins. `Settings.activeMascot` itself, its storage, and the `visualTheme`-switch auto-reassignment logic are untouched (still the fallback mascot for a pack with none of its own) — only the settings-page widget to hand-pick it directly is gone. Onboarding copy updated to stop mentioning "your mascot" as something to configure there.
...
- Bundled pack content (name, exercise labels/categories) translated fr/en on language switch
- Pack-level mascot picker in the editor — bundled mascots or a locally-uploaded image, no JSON import required
...
4. **Sprint 4 — done, 2026-08-27** — XP bar + `pack_progress` table (3.4). `storage.addXp(planId, amount)` / `getPackProgress(planId)`, cleaned up on `deletePlan`; `recordAndHide()` in `main/index.js` credits `XP_PER_EXERCISE = 10` to the active pack on every "done" (tracked via a new `currentPlanId`, mirroring `currentExercise`/`currentMascot`). Mascot growth stages resolved via a small `resolvePackMascotStage(mascot, level)` helper — duplicated on purpose once in `shared/mascots.js` (dashboard/overlay, browser context) and once in `main/index.js` (Node context, feeds the overlay's `payload.mascotImage`) since the two can't share a module. Gallery card gains an XP bar (`xp % 100` width, "Niveau N" label) below the exercise count. **SideCat and SideTama shipped as real bundled packs** (`packages/core/src/exercises/sidecat.json`, `sidetama.json`, exercises only, `color` field set per the Sprint 6 proposal) — no `mascot`/`stages` yet, no cat/tamagotchi art exists to ship; they fall back to the global active mascot in the gallery and overlay exactly like SideGym does today, and the `stages` mechanism itself is fully wired and tested (level → stage selection logic, verified in Sprint 6). **Caveat found while writing Sprint 7's doc**: `PackMascot.imagePath` is used everywhere as an *absolute filesystem path* (`file://${imagePath}` in the overlay payload, decoded-to-`userData` for imports) — that only exists today for imported/custom packs (decoded from base64 at import time). A *bundled* pack's mascot has no equivalent resolution step: `loadPack()`/`listBundledPacks()` in `packages/core` just spread the JSON as-is, with no knowledge of `packages/app/assets/` (deliberately — core doesn't depend on the Electron app's layout). So a bundled pack JSON can't just add a `"mascot": {"imagePath": "sidecat.png"}` key and expect it to resolve; something in `packages/app/src/main/index.js` would need to turn a bundled mascot's relative filename into an absolute path against `ASSETS_DIR` before it reaches `showExercise()`/the dashboard. Not built — nothing exercises it yet (no bundled pack ships a mascot), and building it now for a feature with zero current users would be speculative. Left as a documented gap in `plan-community-packs.md` instead, to close the day real SideCat/SideTama art actually shows up. Verified with a debug-injected XP credit + scripted screenshot: SideGym at 250 XP → "Niveau 3", bar at 50%; SideCat at 30 XP → "Niveau 1", bar at 30%. Core suite 65/65 (4 new tests for `addXp`/`getPackProgress`/deletion cleanup).
5. **Sprint 5 — done, 2026-08-27** — Official logo integrated in the README (3.5, logo part only); base palette identified.
6. **Sprint 6 — done, 2026-08-27** — Per-pack color system wired into the gallery, XP bar, and overlay (3.5, remainder). `resolvePackColor(pack)` — `pack.color` if set, else a stable HSL hue hashed from `pack.id` — added once to `shared/mascots.js` (dashboard) and mirrored once in `main/index.js` (overlay payload), same duplication rationale as `resolvePackMascotStage`. Gallery: each card gets `--pack-accent` as an inline style (`renderPlansGrid()`), consumed by `.plan-card`'s border (`color-mix()` blend with the existing `--border-card`) and `.plan-card-xp-fill`'s background. Overlay: `showExercise()` adds `payload.packColor`, the renderer sets/clears `--pack-accent` on `<html>`, consumed by `style.css` (`.mascot`'s drop-shadow, `.btn-secondary`'s border/text) with a `var(--pack-accent, var(--accent-teal))` fallback since the overlay's property is optional (unlike the gallery, which always sets it inline). Deliberately left badges (Officiel/Importé/Actif) on their existing semantic colors rather than retinting them — they signal provenance/status, not pack identity, and repurposing them would lose that signal. Verified visually: SideCat (explicit `color: "#8a6a45"`) shows a brown-tinted card/bar; SideGym (no explicit color) gets a stable green from the hash fallback, consistently reflected in both the gallery and the overlay's mascot halo/button when that pack is active. Core suite unaffected (this sprint is app-only), still 65/65.
7. **Sprint 7 — done, 2026-08-27** — [`docs/plan-community-packs.md`](plan-community-packs.md) (3.6). Documents the PR-based contribution mechanic (already possible with zero extra code thanks to `listBundledPacks()`), the release-gated limitation vs. instant local import, and the declarative-JSON/no-code-review constraints. Also documents, honestly, the bundled-mascot path-resolution gap found while writing it (see the Sprint 4 entry above) — a contributed pack should ship without a `mascot` field until that's built. Cross-linked from `plan-mvp-sidequest.md`'s V1.5 roadmap row (initially deferred over an unrelated file conflict, resolved by Sprint 8 below).
8. **Sprint 8 — done, 2026-08-27** — Pack-level mascot picker (3.8): assign a mascot directly in the pack editor (bundled image or a locally-uploaded one), no JSON import required. Also updated `storage.updatePlan()` to support a third mascot state (`null` = clear) and refactored `dashboard:delete-plan`'s mascot cleanup into a shared `cleanupCustomMascotFile()` helper reused by the three new handlers. Notably, `dashboard:set-plan-mascot-bundled`'s `ASSETS_DIR`-relative-to-absolute path resolution is the exact mechanism Sprint 7's `plan-community-packs.md` flagged as missing for bundled packs declaring their own mascot — a different call site, but the same reusable pattern, making that older gap meaningfully cheaper to close later (still not done — nothing needs it yet).
9. **Sprint 9 — done, 2026-08-27** — Follow-up on Sprint 8, requested once the mascot picker existed: removed the now-redundant global mascot section from Settings (mascot is a pack property now, having two pickers was confusing about which one wins), and closed a real validation gap — `decodeMascotImage()`/`storeMascotFile()` only checked file extension + byte size, never whether the bytes were an actual decodable image or how large its pixel dimensions were. New shared `isValidMascotImageBuffer()` decodes via `nativeImage` (rejects `isEmpty()` results — an extension that lied) and caps at 2048×2048px. Verified live end-to-end (dialog stubbed, since it can't be scripted like a DOM click): a real 1024×1024 mascot PNG accepted, the 2092px-wide SideQuest logo correctly rejected.
