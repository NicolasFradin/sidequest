...
7. **Sprint 7 — done, 2026-08-27** — [`docs/plan-community-packs.md`](plan-community-packs.md) (3.6). Documents the PR-based contribution mechanic (already possible with zero extra code thanks to `listBundledPacks()`), the release-gated limitation vs. instant local import, and the declarative-JSON/no-code-review constraints. Also documents, honestly, the bundled-mascot path-resolution gap found while writing it (see the Sprint 4 entry above) — a contributed pack should ship without a `mascot` field until that's built. **Not yet linked** from `plan-mvp-sidequest.md`'s V1.5 roadmap row as originally planned — that file currently has unrelated, uncommitted local changes (a backlog section) outside this plan's scope; add the cross-link once those are reconciled.
