...
│   │   ├── exercises/       # static exercise data (JSON sides)
...
`core` is reusable independently of the GUI app (e.g. via `npx`), and will become the foundation of the future marketplace "sides" system.
...
**`exercises`** (sides, declarative JSON format — never executable code, see 3.3)
...
### 3.3 Side format (extensible from the MVP)

A side is a declarative JSON file (never executed JS), e.g.:
...
This choice (declarative data, no code) is what will later let us open side creation up to third-party creators without any security risk (no JS sandboxing to worry about).
...
1. **Sprint 1 — Monorepo skeleton**: pnpm workspaces setup, `core` with scheduler + SQLite storage + 1 "sport" side (10-15 exercises). Scheduler unit tests.
...
| V1 | Exercise generation via an LLM (remote API key, Claude Code/Codex CLI bridge, or local Ollama), personalized programs — see [`plan-llm-side-generation.md`](plan-llm-side-generation.md) |
| V1+ | Local-first marketplace groundwork: custom side import (own exercises + own mascot), sides gallery in the dashboard, per-side XP/gamification (incl. SideCat, SideTama concept sides), official SideQuest logo & per-side color system, community side contribution via PR as a lightweight precursor to the V1.5 registry — see [`plan-marketplace-sides.md`](plan-marketplace-sides.md) |
| V1.5 | Installable side registry (`sidequest install sidegym` / `sideparrot` / `sideyoga` / `sidecodinggame`), still free. SideGym (fitness) ships in the MVP; SideParrot (language learning), SideYoga (relaxation) and SideCodingGame (coding-game practice) are the names reserved for the next sides. A lightweight, PR-based precursor to the registry already exists — see [`plan-community-sides.md`](plan-community-sides.md) |
...
| V3 | Marketplace open to third-party creators (commission): the community can submit its own SideQuests (sides) for review and publishing, not just first-party ones. AI subscription |
...
- ~~**Architecture / flow diagram in the README**~~ — **done 2026-08-27**: Mermaid diagram (rendered natively by GitHub) added to the ["How it works" → "Architecture"](../README.md#how-it-works) section, showing the actual runtime flow — timer / Claude Code hook → `HookServer` → `Scheduler` → overlay window vs. dashboard, SQLite storage, and where the LLM providers (`plan-llm-side-generation.md`) plug in.
- **Custom mascot upload, own image/GIF**: today a custom mascot only arrives bundled inside a full side-import JSON (`plan-marketplace-sides.md` § 3.2), static `png`/`jpg`/`webp` only, animated formats explicitly excluded there. Plan a standalone "change my mascot" flow (independent of importing a whole side) that also allows **animated GIF**, which requires nailing down a format spec before building it: max file size (today's static cap is ~3MB decoded, an animated GIF budget needs its own number), max canvas dimensions, max frame count / duration (a mascot must stay a small idle-loop, not an arbitrary video), and how it renders in the overlay (`<img src="data:image/gif;...">` animates natively, no extra JS needed, unlike the sprite/Rive options considered for *bundled* mascots in section 6's V0.5+ row — worth cross-referencing so both animated-mascot efforts don't diverge on format).
- ~~**New quest side idea — SideMamie (check in on your grandma)**~~ — **done 2026-08-27**: shipped as a bundled side (`packages/core/src/exercises/sidemamie.json`, no dedicated mascot yet — falls back to the active global mascot like SideGym/SideCat/SideTama, same gap noted in `plan-community-sides.md` § "Mascot image, if any"), a 30-second nudge to text or call a grandparent between two prompts, in the same spirit as SideCat/SideTama's care-action sides. SideParrot/SideYoga/SideCodingGame remain reserved-only names (section 6, V1.5).
- **Strip metadata from every image in the repo**: mascots, branding source art (`docs/branding/`), app/tray icons, and the README/dashboard logo may carry EXIF/tool metadata (creation software, timestamps, author/GPS if any) baked in by whatever generated or exported them. Run a one-off pass (e.g. `exiftool -all= -overwrite_original` or `sips`) over every tracked `.png`/`.jpg` and commit the stripped versions — no functional change, just avoids shipping incidental provenance data with the app.

## 9. Visual identity (adapted from the provided template)

Based on the provided "Ronnie Coleman App" style guide (designed for mobile), adapted here for a desktop app (overlay + dashboard). This section predates the SideQuest rebrand and the skin system (section 6, V0.5.1) — the palette below is effectively what the Miami 80's skin's own table reuses in `plan-theme-global.md`. Since 2026-08-27 the project also has an official SideQuest logo (`docs/branding/SideQuest-logo.png`) with its own base brand palette (cyan/cream/dark, additive on top of the 4 skins, not a replacement) — see [`plan-marketplace-sides.md`](plan-marketplace-sides.md) section 3.5 for the extracted colors and the per-side color system built on top of it.
