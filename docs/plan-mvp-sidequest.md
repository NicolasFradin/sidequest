...
| V1.5 | Installable pack registry (`sidequest install sidegym` / `sideparrot` / `sideyoga` / `sidecodinggame`), still free. SideGym (fitness) ships in the MVP; SideParrot (language learning), SideYoga (relaxation) and SideCodingGame (coding-game practice) are the names reserved for the next packs. A lightweight, PR-based precursor to the registry already exists — see [`plan-community-packs.md`](plan-community-packs.md) |
...
- ~~**New quest pack idea — SideMamie (check in on your grandma)**~~ — **done 2026-08-27**: shipped as a bundled pack (`packages/core/src/exercises/sidemamie.json`, no dedicated mascot yet — falls back to the active global mascot like SideGym/SideCat/SideTama, same gap noted in `plan-community-packs.md` § "Mascot image, if any"), a 30-second nudge to text or call a grandparent between two prompts, in the same spirit as SideCat/SideTama's care-action packs. SideParrot/SideYoga/SideCodingGame remain reserved-only names (section 6, V1.5).
