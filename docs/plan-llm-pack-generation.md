...
- **Text-only generation.** No mascot *image* generation — a generated pack has no image-gen mascot; it falls back to the existing hash-based accent color (`plan-marketplace-packs.md` § 3.5) or the user assigns one of the bundled mascots afterward. Explicitly deferred, not forgotten: no provider here can produce an image at all (Anthropic's Messages API and the OpenAI Chat Completions endpoint used by `openai-api.ts` are both text-only; the CLI bridges and Ollama's `/api/generate` are text-only too — real image generation would need a dedicated endpoint like OpenAI's `images/generations`, a separate integration not built).
  - **2026-08-27, lightweight follow-up**: an optional "describe your mascot" textarea was added to the Generate modal. It doesn't generate an image — the description is folded into the same text prompt, and the model may return an extra `mascotIdea` string (`parsePackJson`, one sentence, capped at 300 chars) describing a mascot *concept* in words. Shown once in the plan editor next to the mascot picker (`planEditor.aiMascotIdea`, magenta "AI" styling to match the `generated` pack badge) so the user can pick a matching mascot by hand. Never persisted to the pack/DB — it's a one-shot UI hint, not a `PackMascot`.
