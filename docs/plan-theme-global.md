...
- **`docs/branding/`** reorganized into a subfolder per theme (`sidegym/miami-80s/`, `sidegym/military-camo/`, `sidegym/roman-empire/`, `sidegym/dragonball/`) — see `docs/branding/README.md` for what each subfolder contains and what's still left to integrate. Nested under `sidegym/` since 2026-08-27's branding reorg ([`plan-marketplace-packs.md`](plan-marketplace-packs.md) § 3.9): these 4 skins were designed against the SideGym fitness-coach identity, so the branding folder now groups source art per *pack*, themes included, rather than keeping themes at the top level.
